...
export default function ChatPanel({ token, activeReportId }: ChatPanelProps) {
  ...
  return (
    <section className="frost rounded-2xl p-4">
      <h2 className="section-title text-sm uppercase tracking-[0.2em] text-cyan">Q/A</h2>
      <div className="mt-4 h-72 space-y-3 overflow-y-auto rounded-xl border border-white/10 bg-black/20 p-3">
        {messages.length === 0 ? (
            ...
            <div key={`${msg.role}-${i}`} className={msg.role === "user" ? "text-right" : "text-left"}>
              <span
                className={`inline-block max-w-[85%] rounded-xl px-3 py-2 text-sm ${
                  msg.role === "user" ? "bg-cyan text-ink" : "bg-white/10 text-sand"
                }`}
              >
                {msg.content}
              </span>
      ...
      <form className="mt-3 flex gap-2" onSubmit={onSubmit}>
        <input
          className="w-full rounded-xl border border-white/20 bg-black/25 px-3 py-2 text-sm outline-none focus:border-cyan"
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder="Ask from selected report..."
        />
        <button className="rounded-xl bg-mint px-4 py-2 text-sm font-semibold text-ink" disabled={loading}>
          {loading ? "..." : "Send"}
        </button>
      </form>
    </section>
